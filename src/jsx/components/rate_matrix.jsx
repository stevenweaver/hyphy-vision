...
class RateMatrix extends React.Component {
  componentDidUpdate(prevProps){
    ...
    var margin = {top: 50, right: 50, bottom: 50, left: 50},
      ...
      color_min = '#F8F8F8',
      color_max = '#00A99D',
      width = heatmap_dimension + colorbar_padding + colorbar_width + margin.left + margin.right,
      height = heatmap_dimension + margin.top + margin.bottom;

    var svg = d3.select('#dm-rate-matrix')
      .attr('width', width)
      .attr('height', height);
    ...
    var matrix = svg.append('g')
        ...
        matrix_scale = d3.scale.linear()
          .domain([0, number_of_characters])
          .range([0, heatmap_dimension]),
        colorbar_scale = d3.scale.linear()
          .domain([0, q_max])
          .range([color_min, color_max]),
        colorbar_axis_scale = d3.scale.linear()
          .domain([q_max, 0])
          .range([0, heatmap_dimension]);
    ...
    var colorbar = svg.append("g")
      .attr('transform', 'translate('+(margin.left+heatmap_dimension+colorbar_padding)+','+margin.top+')');
    ...
    for(i = 0; i < number_of_characters; i++){
      for(j = 0; j < number_of_characters; j++){
        ...
        var g = matrix.append("g")
          .attr('transform', 'translate(' + matrix_scale(i) + ',' + matrix_scale(j) + ')');
        g.append('rect')
          .attr('x', 0)
          .attr('y', 0)
        ...
        if (i != j ){
          g.append('text')
            .attr('transform', 'translate(' + matrix_scale(0.5) + ',' + matrix_scale(0.5) + ')')
